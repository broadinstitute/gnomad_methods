...
class TestDeNovoMutation:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ([152, 0, 283], [0, 99, 198], [0, 63, 126], True, False, False, 7.55e-02,
             100 / 3e7, 0.198),
            ([99, 50, 0], [0, 99, 198], [0, 99, 198], False, True, False, 0.005,
             100 / 3e7, 1),
            ([2326, 140, 0], [0, 40, 80], [0, 40, 80], False, False, True, 1.97e-04,
             100 / 3e7, 0.297),
            ([99, 50, 0], [0, 99, 198], [0, 99, 198], False, True, False, 0.005,
             100 / 3e7, 1),
            ([99, 50, 0], [0, 99, 198], [0, 99, 198], False, False, True, 0.005,
             100 / 3e7, 1),
            ([2, 0, 230], [0, 0, 0], [0, 0, 0], True, False, False, 2.03e-02,
             100 / 3e7, 0)
        ]
    )
    ...
    def test_default_get_de_novo_expr_fail_conditions(self):
        ...
        proband_expr = hl.struct(GT=hl.call(0, 1), AD=[9, 2], DP=11, GQ=2, PL=[2, 0, 230])
        ...
        result_expr = default_get_de_novo_expr(
            locus, alleles, proband_expr, father_expr, mother_expr, is_xx_expr,
            freq_prior_expr
        )
        ...
        expected_result_expr = hl.struct(
            ...
            fail_reason=hl.set({"min_de_novo_p", "min_proband_ab", "min_proband_gq", "parent_sum_ad_0"}),
        )
