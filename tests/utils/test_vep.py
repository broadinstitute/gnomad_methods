...
def test_get_most_severe_consequence_expr():
    ...
    assert hl.eval(result) == 'splice_region_variant', "Expected 'splice_region_variant'"
...
def test_add_most_severe_consequence_to_consequence():
    ...
    assert hl.eval(result.most_severe_consequence) == hl.eval(get_most_severe_consequence_expr(tc_expr.consequence_terms)), "Expected 'missense_variant'"
    ...
    assert hl.eval(result.most_severe_consequence) == hl.eval(get_most_severe_consequence_expr(tc_expr.consequence_terms, custom_csq_order)), "Expected 'synonymous_variant'"

    # Test with csq_expr that contains a consequence not in csq_order
    tc_expr = hl.struct(consequence_terms=['non_existent_consequence'])
    result = add_most_severe_consequence_to_consequence(tc_expr)
    assert hl.eval(result.most_severe_consequence) == hl.eval(get_most_severe_consequence_expr(tc_expr.consequence_terms)), "Expected None for non-existent consequence"
...
def test_add_most_severe_csq_to_tc_within_vep_root():
    ...
    assert 'most_severe_consequence' in result_mt.vep.transcript_consequences[0].keys(), "Expected 'most_severe_consequence' field"

    # Check that the most_severe_consequence is correct
    assert result_mt.vep.transcript_consequences[0].most_severe_consequence.take(1)[0] == 'missense_variant'
...
def test_filter_to_most_severe_consequences():
    ...
    assert hl.eval(result.most_severe_consequence) == 'frameshift_variant', "Expected 'frameshift_variant'"
    ...
    assert hl.eval(result.consequences[0].consequence_terms) == ['frameshift_variant', 'synonymous_variant'], "Expected ['frameshift_variant', 'synonymous_variant']"
...
def test_filter_vep_transcript_csqs_expr_loftee():
    ...
    assert hl.eval(hl.all(result.map(lambda csq: hl.is_missing(csq.lof_flags) | (csq.lof_flags == '')))), "Expected no LOFTEE flags"

    # Test with loftee_labels and no_lof_flags
    result = filter_vep_transcript_csqs_expr(csq_expr, loftee_labels=['HC'], no_lof_flags=True)
...
def test_get_most_severe_csq_from_multiple_csq_lists():
    ...
    assert hl.eval(result.most_severe_consequence) == 'missense_variant', "Expected 'missense_variant'"
    ...
    assert hl.eval(hl.len(result.transcript_consequences)) == 2, "Expected 2 transcript consequences"
