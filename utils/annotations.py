...
def faf_expr(
    ...
    - For each population, with adh criteria
    ...
    2. An array expression containing the metadata for each of the array elements, in the same format as that produced by annotate_freq.
    ...
    :rtype: Tuple(ArrayExpression, ArrayExpression)
    """
...
def annotate_freq(
    ...
    _freq_meta_expr = hl.struct(**additional_strata_expr) if additional_strata_expr is not None else hl.struct()
