...
def test_get_most_severe_csq_from_multiple_csq_lists():
    ...
    assert (
        hl.eval(result.most_severe_consequence) == "splice_acceptor_variant"
    ), "Expected 'splice_acceptor_variant'"
    ...
    # Check the prioritize_loftee_no_flags.
    result = get_most_severe_csq_from_multiple_csq_lists(
        vep_expr, prioritize_loftee_no_flags=True
    )

    # Check the most_severe_consequence.
    assert (
        hl.eval(result.most_severe_consequence) == "stop_gained"
