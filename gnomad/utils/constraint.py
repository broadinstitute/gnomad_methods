...
def downsampling_counts_expr(
    ...
    def _get_criteria(i: hl.expr.Int32Expression) -> hl.expr.Int32Expression:
        """
        Return 1 when variant meets specified criteria (`singleton` or `max_af`), if 
        requested, or with an AC > 0.
...
def build_models(
    ...
    high_coverage_scale_factor = high_cov_ht.aggregate(
        ...
        / hl.agg.sum(
            high_cov_ht.possible_variants * high_cov_ht.mu_snp
        )
    )
    ...
    low_cov_group_ht = low_cov_ht.group_by(
        ...
        / (
            ...
            * hl.agg.sum(
                low_cov_ht.possible_variants * low_cov_ht.mu_snp
            )
        )
    )
...
def get_all_pop_lengths(
    ...
    The minimum array length will be used to determine the number of plateau models 
    built for each population.
